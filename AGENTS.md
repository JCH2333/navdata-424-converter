...
- 解决方式：仍先要求唯一的 1e-6 度精确匹配；仅在没有精确候选时，接受唯一的 0.02 海里候选。两个或更多近距候选仍拒绝，航路的任一端点拒绝则整条航路不写入。
- 自动化与验证：`test_inserts_airway_when_one_target_is_within_source_coordinate_precision` 覆盖唯一近距接受，既有歧义拒绝测试保持通过。以同一 CSV/PDF 模型生成 `output/candidate-2608-airways-near`：166 条航路、409 条航段得到来源投影，317 条拒绝；`integrity_check=ok`，航路表为 `10026/159445`（参考 `10338/163724`）。候选 SHA-256 `0234f893d012d6596051c97835aa65680d1acbd689bce441c733a2c2c430ebab` 仍不等于参考，禁止部署或发布。

## 2026-08-08 RTE_SEG.csv 来源航路投影

- 适用范围：2608 `RTE_SEG.csv` 中新增、且官方 Fenix 全球模板尚不存在的航路。每个原始航段同时提供 `CODE_DIR`、两端定位点标识和精确 DMS 坐标；这些字段必须整体保留在中间模型，不能只依靠同名定位点或参考库行解析目标 ID。
- 解决方式：Fenix 适配器只在每条航路的全部端点都能以“标识 + CSV 坐标”唯一解析为已写入的 `Waypoints` 时，才写入 `Airways` 和 `AirwayLegs`。`F/B/X` 分别投影为正向、反向和两条独立的正反向路径；全部写入 NAIP 2608 航路层 `B`，并在路径边界写入 `IsStart/IsEnd`。坐标碰撞、缺坐标或未知方向会拒绝整条航路，不用 rowid、最近同名点或参考记录补齐。
- 自动化与验证：`test_inserts_source_airway_with_printed_bidirectional_segments` 覆盖双向顺序和边界，`test_rejects_airway_without_unique_source_coordinate_target` 覆盖同坐标歧义拒绝。重新解析 CSV/PDF 后生成 `output/candidate-2608-airways`：142 条航路、336 条航段得到来源投影，341 条因目标端点不唯一或缺失而拒绝；`integrity_check=ok`。与只读参考相比，航路表计数由 `9860/159036` 增至 `10002/159372`（参考 `10338/163724`）；新增 336 个方向签名中 95 个相同。其余来源明确但与参考不同，保持来源驱动，不按差分删除；候选 SHA-256 `f26e48563139f3d51b24534c573e7ae23d7b67d0430de822991d2b6d228ecc21` 仍不等于参考，禁止部署或发布。
...
- 解决方式：IF 读取定位点后的首个独立整数高度；RF 仅在弧心半径右方读取整数高度，明确排除方括号内的弧半径。`test_extracts_if_and_rf_altitudes_without_using_rf_arc_radius` 固化两种来源行；证据缓存版本升为 30。
- 验证：完整 `pytest` 为 `118 passed`。仅 CSV/PDF 重解析后生成 `output/candidate-2608-if-rf-altitudes`，`integrity_check=ok`；高度非空航段从 `521547` 增至 `524051`（参考 `526798`），新增 `IF=2042`、`RF=462` 条。以机场/类型/名称/跑道/航段序号对齐，新增值中 `1475` 条同时匹配参考航段类型和高度，另有 `43` 条高度一致但已有程序分段不同；不据此回填剩余字段。候选 SHA-256 `7a5e45f30008257851cee6398c751fd35478bb1228658239646f6f09ba59501d` 仍不等于参考，禁止部署或发布。
...
- 解决方式：进近标题仅允许紧跟在跑道后的受限 `RNP ILS` 或 `AR` 性能前缀；新增的无“进近”过渡规则仍要求 `RWYxx 过渡` 和明确固定点。三种版式均规范为 `进近过渡`，不根据航段形状或参考库猜测。
- 自动化证据：`test_extracts_database_approach_transition_with_printed_rnp_ils_or_ar_prefix` 覆盖三种原文标题；针对上述三个 PDF 的 PyMuPDF 坐标重建确认模型输入文本。完整 CSV/PDF 重解析和候选差分尚待执行；在其完成前没有生成可部署产物。

## 2026-08-08 带性能前缀进近过渡的完整候选验证

- 完整 `load_naip` 仅使用 `424源数据\\2608\\2608` CSV/PDF 和本地 PDF 缓存，模型写入被忽略的 `output\\model-2608-prefixed-approach.pickle`（18.9 MB）。转换未传入参考路径，生成 `output\\candidate-2608-prefixed-approach`；SQLite `integrity_check=ok`，`Airports=17346`、`Terminals=100722`、`TerminalLegs/Ex=839889`。
- 只读 `fnx2608N` 差分：候选 SHA-256 `531fd88dca36195c9e623529e285a045802f4818d9b8e8a51c636325942c2eeb`，参考 SHA-256 `ca9cdd72b80d46b4c28e884bcd2ecf4b29bc54489704771d7908b32c6e3c510f`，仍不字节相同。终端业务键缺失为 986，额外从上一候选的 104 降至 101。
- 表计数减少的三条均为 `ZUAL/15` 的旧无类型伪程序 `IS95A`、`IS96A`、`IS98A`；参考库没有它们。PDF 明确把它们列为 `R15` 的进近过渡，当前 IAP 投影需完整主进近与 MAPT 才写入，因此不再将它们冒充独立终端程序。该变化没有参考回填，候选仍为不可部署诊断件。

## 2026-08-08 合并进近/复飞编码段的跑道 MAP 边界

- 适用范围：RNP AR 等进近的数据库编码页将主进近和后续复飞航段列在同一“进近”段，而唯一匹配的仪表进近图文字明确出现“复飞程序”。`ZSWY/R21` 的编码序列在 `RW21C` 后继续为 `WY420...`，索引唯一关联 `ZSWY-9B.pdf` 的 `RNP RWY21(AR)`；图页来源文字明确有“复飞程序”。
- 解决方式：`ProcedureChart.has_missed_approach` 保留原生 PDF 文字事实；只有唯一匹配图具备该事实，且编码主段仅有一个与本跑道匹配的 `RWxx` 固定点并且该点后仍有航段时，才将该点作为 MAP 边界，之后的航段投影为复飞。多个图、无标题、零或多个跑道点一律保持拒绝。`test_retains_explicit_missed_approach_heading_from_chart_text` 与 `test_splits_combined_iap_only_at_unique_runway_fix_with_explicit_missed_chart` 覆盖证据和拒绝边界。
- 验证：以仅 CSV/PDF 重解析的 `output\\model-2608-runway-map.pickle` 生成候选，`integrity_check=ok`，`Terminals=100760`、`TerminalLegs/Ex=840767`。相对上一候选新增 38 个 IAP 业务键，均在参考库中，未增加参考不存在的键；终端缺失由 986 降至 948，额外键仍为 101。逐航段归一化比较显示 38 条尚不完全一致，主要是来源表高度/限速提取和过渡分段缺口，故 SHA-256 `76dacc692112c5ed42a2ecfb97b27b9a6b06d1a3c05b44b8e43493a10c9ff543` 仍不等于参考 `ca9cdd72b80d46b4c28e884bcd2ecf4b29bc54489704771d7908b32c6e3c510f`；不可部署或发布。

## 2026-08-08 数据库编码表的错位基线与分隔进近标题

- 适用范围：2608 NAIP 数据库编码表中，左侧旋转/残留文字与同一标题行的文字对象具有不同 y 坐标，以及标题将 `进近 过渡` 分为两个对象、使用 `进近、复飞` 的版式。`Terminal/ZSSH/ZSSH-4Z03.pdf` 的原生文字对象明确给出 `RWY22`、`进近`、`过渡`、`SH705`；其中左侧残留文字 y=252.26、标题 y=253.70、中文词 y=254.80。原规则只比较行首 y 值，导致中文词被拆到下一行，随后把 `RWY22 SH705` 误作空类型 `SH7-05`。
- 解决方式：`_positioned_database_text` 按当前行任一对象的 2.5 点同基线窗口归并，而非固定比较行首对象；进近标题允许 `进近 过渡` 的对象间空格和 `进近、复飞` 连接符，再规范化为既有内部类型。`test_rebuilds_database_heading_when_left_margin_text_has_a_shifted_baseline` 同时覆盖坐标重建、过渡和复飞分段。
- 验证：完整 CSV/PDF 重解析后，`ZSSH/R22` 保留 `SH705 -> SH703`、`SH704 -> SH703` 过渡、主进近及复飞。未传入参考库生成 `output/candidate-2608-baseline-cluster`，`integrity_check=ok`，`Terminals` 从 `100685/101618` 增至 `100725/101618`，`TerminalLegs/Ex` 从 `839456/845147` 增至 `839900/845147`，航段缺口从 5691 降至 5247。只读参考差分显示其只有 `SH704` 过渡；来源中明确存在 `SH705`，因此不按参考删除。候选 SHA-256 `d28bb9e90c48d78b11616fc8840cd7115ee5e9ef32b2b8d009744d5beac56ffc` 仍不等于参考 `ca9cdd72b80d46b4c28e884bcd2ecf4b29bc54489704771d7908b32c6e3c510f`，不可部署或发布。
